...
class SceneryPainter extends CustomPainter {
  ...
  SceneryPainter({
    ...
    required this.drawAfternoonSun,
  ...
  void _drawAfternoonSun(Canvas canvas) {
    final sunCenter = _center - Offset(-2 * _center.dx / 4, _center.dy / 2);
    final innerRadius = 40.0;
    final outerRadius = innerRadius + innerRadius / 3;
    final sunColor = Colors.yellowAccent;
    _paint.color = sunColor;
  ...
  void _drawMoon(Canvas canvas) {
    final moonCenter = _center + Offset(-_center.dx / 4, -_center.dy / 2);
